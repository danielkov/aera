...
module.exports = class Server {
  ...
  /**
   * use - passes handler to the server's request event
   *
  ...
  /**
   * listen - makes the underlying server listen with the specified options exactly as the original httpServer
   *
   * @param  {number} port        - port on which connections will be accepted
   * @param  {string} hostname    - hostname to which connections will be accepted
   * @param  {number} backlog     - maximum queue of pending connections, actually decided by `tcp_max_syn_backlog` on Linux
   * @param  {function} callback  - the handler that gets added to the event stack, triggered by `listening` event
   * @return {this}         returns the instance
   */

  listen (...params) {
    (typeof params[0] !== 'number' && this.port) && params.unshift(this.port)
    this.use(this._handler.bind(this))
  ...
  _handler (req, res) {
    ...
    let { exceptions } = this
    parseResult({req, res, result, exceptions})
  }
